...
function GamePacketSender:sendGameStateInfo(state)
    local params = { state = state }
    local data = DataBuilder.new():fromTable(params):getData()
    PacketSender:broadCastLuaCommonData("GameStateInfo", data)
end

function GamePacketSender:sendShowResultAward(rakssid, data)
    PacketSender:sendLuaCommonData(rakssid, "ShowResultAward", json.encode(data))
end

function GamePacketSender:sendWaitGameStartCountDown(time)
    local params = { time = time }
    local data = DataBuilder.new():fromTable(params):getData()
    PacketSender:broadCastLuaCommonData("WaitGameStartCountDown", data)
end

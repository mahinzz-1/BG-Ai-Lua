...
function RandomUtil:init(data, prob, alias)
    local nums = #data
    local small = {}
    local large = {}
    for i = 1, nums do
...
function RandomUtil:generation(prob, alias)
...
function RandomUtil:initRandomSeed(remark, data)
    self.prob[remark], self.alias[remark] = self:init(data, {}, {})
end

function RandomUtil:get(remark)
    return self:generation(self.prob[remark], self.alias[remark])
end

function RandomUtil:setRandomSeed()
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
end

function RandomUtil:randomItemByWeight(num, pool, ifReduce)
    if type(num) ~= "number"
            or type(pool) ~= "table"
            or type(ifReduce) ~= "boolean" then
        return nil
    end
    local sumOfWeight = self:getSumOfWeight(pool)
    local result = {}

    local function randomItem()
        if ifReduce then
            sumOfWeight = self:getSumOfWeight(pool)
        end
        local rs = HostApi.random("RandomPool", 0, sumOfWeight)
...
function RandomUtil:getSumOfWeight(pool)
    local sumOfWeight = 0

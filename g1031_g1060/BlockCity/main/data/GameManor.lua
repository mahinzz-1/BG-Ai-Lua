...
function GameManor:ctor(manorIndex, userId)
    ...
    self.visitor = {}
    self.manorScore = 0
    self.mailBoxEntityId = 0
    self:addMailBoxToWorld()
end
...
function GameManor:removeFromWorld()
    ...
    self:removeMailBoxFromWorld()
end

function GameManor:getManorScore()
    return ManorLevelConfig:getScoreByLevel(self.level)
end
...
function GameManor:isCanUpgrade()
...
function GameManor:upgradeLevel()
    ...
    ManorLevelConfig:createHouseManager(self.userId, self.manorIndex, self.level)
...
function GameManor:addMailBoxToWorld()
    local mailBox = MailBoxConfig:getMailBoxById(self.manorIndex)
    if not mailBox then
        return
    end
    local pos = VectorUtil.newVector3(mailBox.x, mailBox.y, mailBox.z)
    local entityId = EngineWorld:addSessionNpc(pos, mailBox.yaw, 104703, mailBox.actorName, function(entity)
        entity:setActorBody(mailBox.actorBody or "body")
        entity:setActorBodyId(mailBox.actorBodyId or "")
    end)
    self.mailBoxEntityId = entityId
end

function GameManor:removeMailBoxFromWorld()
    EngineWorld:removeEntity(self.mailBoxEntityId)
end

--function GameManor:pushMailBoxAreaInfo()
--    local mailBox = MailBoxConfig:getMailBoxById(self.manorIndex)
--    if not mailBox then
--        return
--    end
--
--    AreaInfoManager:push(self.mailBoxEntityId, AreaType.MailBox, mailBox.tipNor, mailBox.tipPre, mailBox.imageNor, mailBox.imagePre, false,
--        VectorUtil.newVector3(mailBox.x1, mailBox.y1, mailBox.z1), VectorUtil.newVector3(mailBox.x2, mailBox.y2, mailBox.z2))
--
--end
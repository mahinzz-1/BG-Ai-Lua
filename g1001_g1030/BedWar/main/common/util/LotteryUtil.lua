...
function LotteryUtil:init(data, prob, alias)
    local nums = #data
    local small = {}
    local large = {}
    for i = 1, nums do
...
function LotteryUtil:generation(prob, alias)
...
function LotteryUtil:initRandomSeed(remark, data) 
    self.prob[remark], self.alias[remark] = self:init(data, {}, {})
end

function LotteryUtil:get(remark)
    return self:generation(self.prob[remark], self.alias[remark])
end

function LotteryUtil:setRandomSeed()

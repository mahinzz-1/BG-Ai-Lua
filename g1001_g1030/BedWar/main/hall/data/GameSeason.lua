...
function GameSeason:onGetUserCurrentSeasonInfo(player, data)
    ...
    GamePacketSender:sendBedWarCurrentSeasonInfo(player.rakssid, honorId, rank, honor, days)

    if player.hasReportSeasonRank == false or player.lastReportSeasonRankId == -1 then
        ReportUtil.reportSeasonRank(player)
    end
...
function GameSeason:onGetUserLastSeasonInfo(player, data)
    ...
    local rank = data.rank
    local honor = data.integral
    GamePacketSender:sendBedWarLastSeasonInfo(player.rakssid, honorId, rank, honor)

...
UpgradeEquipBoxConfig = {}
UpgradeEquipBoxConfig.equipboxs = {}

function UpgradeEquipBoxConfig:init(equipboxs)
    self:initEquipboxs(equipboxs)
end

function UpgradeEquipBoxConfig:initEquipboxs(equipboxs)
    for id, c_item in pairs(equipboxs) do
        ...
        self.equipboxs[tostring(item.id)] = item
    end
end

function UpgradeEquipBoxConfig:getConfigById(id)
    local equipbox = self.equipboxs[id]
    if equipbox then
        return equipbox
    end
    error("EquipBox config error, can't find groupId:" .. id)
end

return UpgradeEquipBoxConfig
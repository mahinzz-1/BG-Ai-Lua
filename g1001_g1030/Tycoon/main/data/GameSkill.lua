...
GameSkill.EFFECT_TYPE_TRANSFER = 5
...
function GameSkill:doAction()
    local creator = PlayerManager:getPlayerByRakssid(self.player.rakssid)
    if creator == nil or creator.isDead then
        return
    end
    ...
    if self.effectType == GameSkill.EFFECT_TYPE_TRANSFER then
    ...
    for i, player in pairs(players) do
        ...
        if self.effectType == GameSkill.EFFECT_TYPE_DAMAGE then
            ...
            player.killerBySkill = self.player.rakssid

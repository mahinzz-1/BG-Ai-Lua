...
function GameRankNpc:savePlayerRankScore(player)
    local score = 0
    if self.key == "score" then
        if player.score > 0 then
            score = player.score
            player.score = 0
        end
    end
    if self.key == "wins" then
        if player.wins > 0 then
            score = player.wins
            player.wins = 0
        end
    end
    if self.key == "No1" then
        if player.no1 > 0 then
            score = player.no1
            player.no1 = 0
        end
    end
...
function GameRankNpc:getDayKey()
    return GameMatch.gameType .. "." .. self.key .. ".week." .. DateUtil.getYearWeek(os.time() - DateUtil.getWeekSeconds())
end

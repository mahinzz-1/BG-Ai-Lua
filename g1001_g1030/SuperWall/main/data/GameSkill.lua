...
function GameSkill:doAction()
    ...
    for i, player in pairs(players) do
        ...
        if self.effectType == GameSkill.EFFECT_TYPE_DAMAGE then
            local damage = self.damage * (1 - player.defenseX) * AttackXConfig:getAttackX(self.player.type, player.type)
            damage = math.max(0.5, damage)
            if player:getHealth() <= damage then
                self.player:onKill(player)
            end
            player:subHealth(damage)
        end
        if self.effectType == GameSkill.EFFECT_TYPE_SPEED then
            player:setSpeedAddition(player.speed + self.damage)

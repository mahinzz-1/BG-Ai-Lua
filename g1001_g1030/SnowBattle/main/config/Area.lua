...
function Area:setMatrix(matrix)
    ...
    for i, v in pairs(matrix) do
        ...
        if tonumber(v[2]) > tonumber(self.maxY) then
            self.maxY = v[2]
        end
...
function Area:inArea(vec3)
    return vec3.y >= tonumber(self.minY) and vec3.y <= tonumber(self.maxY) and (vec3.x >= tonumber(self.minX) and vec3.x <= tonumber(self.maxX) and vec3.z >= tonumber(self.minZ) and vec3.z <= tonumber(self.maxZ))
end
